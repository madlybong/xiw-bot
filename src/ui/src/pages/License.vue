    ...
    <v-row>
      <v-col cols="12">
        ...
        <div v-else-if="license">
            <!-- Warnings -->
            <v-alert v-if="license.machine_mismatch" type="warning" variant="tonal" class="mb-4 text-amber-accent-2 border-amber">
                <div class="font-weight-bold text-h6">Machine Mismatch Detected</div>
                This license is bound to a different machine fingerprint. This may be a violation of the license terms.
            </v-alert>

            <v-alert v-if="license.status === 'expired'" type="error" variant="tonal" class="mb-4 text-red-accent-2 border-red">
            ...
            <v-alert v-if="license.status === 'warning'" type="warning" variant="tonal" class="mb-4 text-amber-accent-2 border-amber">
                    ...
                    <v-row>
                        <v-col cols="12" md="6">
                            <v-list bg-color="transparent">
                                ...
                                <v-list-item title="Billing Cycle" :subtitle="license.billing_cycle"></v-list-item>
                            </v-list>
                        </v-col>
                        <v-col cols="12" md="6">
                            <v-list bg-color="transparent">
                                <v-list-item title="Issued At" :subtitle="license.issued_at.split('T')[0]"></v-list-item>
                                <v-list-item title="Expires At" :subtitle="license.expires_at.split('T')[0]"></v-list-item>
                                <v-list-item>
                                    <template v-slot:title>Machine Binding</template>
                                    <template v-slot:subtitle>
                                        <v-chip size="small" :color="license.machine_mismatch ? 'error' : 'success'" variant="outlined">
                                            {{ license.machine_mismatch ? 'Mismatch' : (license.machine?.bound ? 'Bound' : 'Soft Bound') }}
                                        </v-chip>
                                    </template>
                                </v-list-item>
                            </v-list>
                    ...
                    <v-row>
                         <v-col cols="12" md="6">
                             ...
                             <div class="text-h5 font-weight-bold">
                                 {{ license.limits.max_whatsapp_accounts === -1 ? 'Unlimited' : license.limits.max_whatsapp_accounts }}
                             </div>
                         </v-col>
                         <v-col cols="12" md="6">
