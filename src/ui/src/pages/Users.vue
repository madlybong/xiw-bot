            ...
            <v-table>
                ...
                <tbody>
                    <tr v-for="user in users" :key="user.id">
                        ...
                        <td>
                            <v-chip :color="user.role === 'admin' ? 'purple' : 'blue'" size="small"
                                class="text-uppercase font-weight-bold" label variant="tonal">
                                {{ user.role }}
                            </v-chip>
                        </td>
                        <td>
                        ...
                        <td>
                            <v-btn icon="mdi-pencil" variant="text" color="primary" size="small"
                                @click="openEdit(user)"></v-btn>
                            <v-btn icon="mdi-delete" variant="text" color="error" size="small"
                                :disabled="user.role === 'admin'" @click="deleteUser(user.id)"></v-btn>
                        </td>
                    </tr>
                </tbody>
            </v-table>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-form @submit.prevent="saveUser">
                        <v-row>
                            <v-col cols="6">
                                <v-text-field v-model="formUser.username" label="Username" :disabled="isEdit"
                                    variant="outlined" density="compact"></v-text-field>
                            </v-col>
                            <v-col cols="6">
                                <v-text-field v-model="formUser.password" label="Password"
                                    :placeholder="isEdit ? 'Unchanged' : ''" type="password" variant="outlined"
                        ...
                        <v-row>
                            <v-col cols="6">
                                <v-select v-model="formUser.status" label="Status" :items="['active', 'suspended']"
                        ...
                        <v-row v-if="formUser.role !== 'admin'">
                            <v-col cols="6">
                                <v-text-field v-model.number="formUser.message_limit" label="Message Limit"
                                    type="number" variant="outlined" density="compact"></v-text-field>
                            </v-col>
                            <v-col cols="6">
                                <v-select v-model="formUser.limit_frequency" label="Frequency"
                                    :items="['daily', 'monthly', 'unlimited']" variant="outlined"
                        ...
                        <div v-if="isEdit && formUser.role !== 'admin'" class="mt-4 bg-grey-darken-4 pa-4 rounded">
                            ...
                            <p class="text-caption text-medium-emphasis mb-4">
                                Assign instances and generate a static token. This token is <strong>mandatory</strong>
                                for sending messages via API.
                            </p>

                            <v-select v-model="selectedInstances" :items="instances" item-title="name" item-value="id"
                                label="Assigned Instances" multiple chips variant="outlined"
                                density="compact"></v-select>

                            <div class="d-flex align-center mt-2">
                                <v-text-field v-model="staticToken" label="Static Token" readonly variant="outlined"
                                    density="compact" hide-details class="mr-2"></v-text-field>
